...
async function getRegistrations(status?: string) {
  ...
  try {
    ...
    let url = `${apiUrl}/teacher_registrations`;
    if (status) {
...
const Registers = async ({status}: RegistersProps) => {
  const {success, data, metadata, error} = await getRegistrations(status);
  ...
  return (
    <div className="teacher-registration-report space-y-6">
      ...
      {/* Data Table Section */}
      <div className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">
        <div className='flex justify-between items-center md:p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50'>
