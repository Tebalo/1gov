...
import ProgressIndicator from "./simple-progress-indicator";
            ...
            if(!error){
                ...
                if (result.code === 200 || result.code === 201) {
                    ...
                    toast({
                        title: "Error",
                        variant: "destructive",
                        description: `${result.message || 'Conflict error occurred.'}`
                    });
                } 
            }
        } catch (error) {
    ...
    return(
        <div>
                ...
                <Form {...form}>
                    <form id="flow-action-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem className="space-y-3">
                                    <FormLabel>Choose an action to proceed</FormLabel>
                                    <FormControl>
                                        <RadioGroup 
                                        ...
                                        >
                                            {nextStatus?.map((status) => (
                                                <div key={status} className="flex items-center space-x-2 rounded-lg p-2 hover:bg-gray-100">
                        ...
                        {isSubmitting && (
                            ...
                            totalDuration={(current_status=="Recommended-For-Approval" || current_status=="Recommended-For-Rejection") ? 10000 : 3000}
                            />)
                        )}
                    </form> 
                </Form>
            ...
            <>
                ...
                <AlertDescription>
                    It looks like you don&lsquo;t have the necessary permissions to move this case forward or change its status. If you believe this is a mistake, please reach out to your administrator for assistance.                </AlertDescription>
                </Alert>
            </>)}
        </div>
    )
